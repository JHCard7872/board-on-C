...
int main(void) {
	...
	for (int m = 0; m < 200; m++) { // 보드판 출력 반복.
		...
		for (int j = 0; j < 4; j++)
		{
			...
			{
				gameover(&play[j]);
				printString(11 + 6 * j, 66, 9, Gameover);
			}
		}
		if (play[i].money <= 0) continue;
...
void Pass(struct board* _boardCell ,struct player* Player) {
	...
	printString(12, 138, 14, buf);
	_itoa((_boardCell+Player->location)->pass_value/1000, buf_number, 10);

	if ((_boardCell + Player->location)->pass_value >= 1000 && (_boardCell + Player->location)->pass_value < 10000) {
		printString(12, 152, 1, buf_number); boardpan[12][153] = 'k';
	}
	else if ((_boardCell + Player->location)->pass_value >= 10000 && (_boardCell + Player->location)->pass_value < 100000) {
		printString(12, 152, 2, buf_number); boardpan[12][154] = 'k';
	}
	else if ((_boardCell + Player->location)->pass_value >= 100000 && (_boardCell + Player->location)->pass_value < 1000000) {
		printString(12, 152, 3, buf_number); boardpan[12][155] = 'k';
	}
	else if ((_boardCell + Player->location)->pass_value >= 1000000 && (_boardCell + Player->location)->pass_value < 10000000) {
		printString(12, 152, 4, buf_number); boardpan[12][156] = 'k';
	}
	else if ((_boardCell + Player->location)->pass_value >= 10000000 && (_boardCell + Player->location)->pass_value < 100000000) {
		printString(12, 152, 5, buf_number); boardpan[12][157] = 'k';
	}
	else if ((_boardCell + Player->location)->pass_value >= 100000000 && (_boardCell + Player->location)->pass_value < 1000000000) {
		printString(12, 152, 6, buf_number); boardpan[12][158] = 'k';
	}
...
void gameover(struct player* Player) {
	...
	if (Player->money < 0)
	{
		for (int i = 0; i < Player->haveBoard; i++)
		{
			...
			Player->player_board[i]->get_player = NULL;
		...
		{
			printString(11, 70, 9, gameover);
		}
		if (Player->color == RED)
		{
			printString(17, 70, 9, gameover);
		}
		if (Player->color == GREEN)
		{
			printString(23, 70, 9, gameover);
		}
		if (Player->color == YELLOW)
		{
			printString(29, 70, 9, gameover);
		}
	}
}