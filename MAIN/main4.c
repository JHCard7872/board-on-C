...
void sell(struct board* _boardCell, struct player* Player)//_boardCell은 보드 구조체의 주소, Player는 현재 차레인 플레이어의 구조체 주소.
void buy(struct board* _boardCell, struct player* Player)//위와 동일. 플레이어가 이동한 후에 쓸 것.
void build(struct board* _boardCell, struct player* Player)//위와 동일. 플레이어가 이동한 후에 쓸 것. 
