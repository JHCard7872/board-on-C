...
void print_board(void) { // 보드판 출력10000
	...
	for (int i = 0; i < 41; i++)
	{
		...
		else if (i == 23) // Green 출력해주는 부분
		{
			for (int j = 0; j < 180; j++)
			{
				if ((j == 1 && boardCell[(77 - i) / 4].get_player != NULL)) { printcolor(boardCell[(77 - i) / 4].get_player->color); printf("%c", boardpan[i][j]); }
	...
	if (_player->location == 9) { // 플레이어 위치가 무인도일때
		...
		if (_player->count < 3) {
			...
			if (_player->location == 9)
			{
				for (int i = 0; i < 10; i++)
				{
					boardpan[12][138 + i] = island[i]; // 무인도 몇번째에서 출력
				}
				for (int i = 0; i < 17; i++)
				{
					boardpan[12][150 + i] = island2[i]; // 무인도 번째입니다 출력
				}
	...
	if (d1 == d2) { // 더블일때
		*i += 1;
		...
		if (*i >= 3) { // 더블이 3번이상일경우
			...
			for (int i = 0; i < 29; i++)
			{
				boardpan[12][138 + i] = doubleIsland[i];
			}
			if (k == 1)
			{
				boardpan[10][120] = 'B';
