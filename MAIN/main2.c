...
void print_board(void) { // 보드판 출력10000
	...
	for (int i = 0; i < 41; i++)
	{
		if (i % 4 == 1) // 플레이어 위치를 알려주는 *를 출력해주는 자리
		{
			for (int j = 0; j < 150; j++)
			{
				if (j % 11 == 3 && (i == 1 || i == 37)) { printcolor(BLUE); printf("%c", boardpan[i][j]);  } // *가 출력되는 위치에 따라 색깔 출력
				else if (j % 11 == 4 && (i == 1 || i == 37)) {  printf("%c", boardpan[i][j]);  printcolor(WHITE); }
				else if (j % 11 == 5 && (i == 1 || i == 37)) { printcolor(RED); printf("%c", boardpan[i][j]); }
				else if (j % 11 == 6 && (i == 1 || i == 37)) { printf("%c", boardpan[i][j]); printcolor(WHITE); }
				else if (j % 11 == 7 && (i == 1 || i == 37)) { printcolor(GREEN); printf("%c", boardpan[i][j]);  }
				else if (j % 11 == 8 && (i == 1 || i == 37)) { printf("%c", boardpan[i][j]); printcolor(WHITE); }
				else if (j % 11 == 9 && (i == 1 || i == 37)) { printcolor(YELLOW); printf("%c", boardpan[i][j]);  }
				else if (j % 11 == 10 && (i == 1 || i == 37)) { printf("%c", boardpan[i][j]); printcolor(WHITE); }
