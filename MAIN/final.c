...
struct player play[4] = {
		{0, BLUE, 20000, 0, 0, NULL},
		{0, RED, 1000000,0, 0, NULL},
		{0, GREEN, 10000,0, 0, NULL},
		{0, YELLOW,5000,0, 0, NULL}
};
...
int main(void) {
	...
	for (int n = 0; n < 4; n++)
	{
		if (play[n].money > 0)
		{
			if (n == 0) printf("Blue 승리!!");
			else if (n == 1)printf("Red 승리!!");
			else if (n == 2) printf("Green 승리!!");
			else printf("Yellow승리!!");
		}
	}
}
...
void sell(struct board* _boardCell, struct player* Player) {
	...
	if (_getch() == 'y')
	{
		printf("팔고 싶은 땅의 번호를 입력하세요 : ");
		int number;
		scanf_s("%d", &number);
		if ((_boardCell + number)->get_player->color != Player->color) return 0;
...
void Pass(struct board* _boardCell, struct player* Player) {
	...
	if ((_boardCell + Player->location)->pass_value < Player->money)
	{
		printf("통행료 지불! -%dk", (_boardCell + Player->location)->pass_value / 1000);
